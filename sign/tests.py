...
class ModelTest(TestCase):
    ...
    def test_guest_models(self):
        result = Guest.objects.get(phone='15172365963')
        self.assertEqual(result.realname, 'claudia')
...
class GuestManageTest(TestCase):
    ...
    def setUp(self):
        ...
        Guest.objects.create(realname='claudia',phone='15172365963',email='claudia@mail.com',sign=0,event_id=1)
        self.login_user = {'username':'admin', 'password':'admin123456'}
    ...
    def test_event_manage_success(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertIn(b'claudia',response.content)
        self.assertIn(b'15172365963', response.content)

    def test_guest_manage_search_success(self):
        ''' 测试嘉宾搜索 '''
        response = self.client.post('/login_action/', data=self.login_user)
        response = self.client.post('/search_phone/', {'phone': "15172365963"})
        self.assertEqual(response.status_code, 200)
        self.assertIn(b"claudia", response.content)
        self.assertIn(b'15172365963', response.content)
...
class SignIndexActionTest(TestCase):
    ...
    def setUp(self):
        ...
        Guest.objects.create(realname='claudia',phone='15172365963',email='claudia@mail.com',sign=0,event_id=1)
        Guest.objects.create(realname='alen', phone='13172365963', email='alen@mail.com', sign=1, event_id=2)
        self.login_user = {'username':'admin', 'password':'admin123456'}
    ...
    def test_sign_index_action_phone_or_event_id_error(self):
        ''' 手机号或发布会id错误 '''
        response = self.client.post('/login_action/',data=self.login_user)
        response = self.client.post('/sign_index_action/2/',{'phone':'13172365961','event_id':3})
        self.assertEqual(response.status_code, 200)
    ...
    def test_sign_index_action_user_sign_has(self):
        ''' 用户已签到 '''
        response = self.client.post('/login_action/', data=self.login_user)
        response = self.client.post('/sign_index_action/2/',{'phone': '13172365963'})
        self.assertEqual(response.status_code, 200)
    ...
    def test_sign_index_action_sign_success(self):
        ''' 签到成功 '''
        response = self.client.post('/login_action/', data=self.login_user)
        response = self.client.post('/sign_index_action/1/',{"phone": '15172365963'})
        self.assertEqual(response.status_code, 200)
